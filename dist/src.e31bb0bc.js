...
var createDomElement = function createDomElement(tag, attributes, target) {
...
var error = null;
var cartItems = [];
var buyCart = document.getElementById("cart");
buyCart.addEventListener("click", function (e) {
...
function removeFromCart(itemId) {
  var cartItem = data.filter(function (item) {
    return item.id == itemId;
  });
...
function getData() {
  loading = true;
  render();
  fetch('http://makeup-api.herokuapp.com/api/v1/products.json?product_type=nail_polish').then(function (res) {
    return res.json();
  }).then(function (json) {
    loading = false;
    data = json;
...
function drawCard(element, i) {
  ...
  var price = createDomElement("h4", {
    innerText: element.price
  });
  var pic = createDomElement("img", {
    src: element.image_link
  });
  var brand = createDomElement("h3", {
    innerText: element.brand
  });
  ...
  addToDom("#app", card);
  Array.from([pic, price, brand, buyBtn, cartBtn]).forEach(function (content) {
...
function drawCartCard(el, i) {
  ...
  document.querySelector("#cart-items").appendChild(card);
  document.querySelector("#cart-items > div:nth-child(".concat(i + 1, ")")).appendChild(pic);
  document.querySelector("#cart-items > div:nth-child(".concat(i + 1, ")")).appendChild(price);
  document.querySelector("#cart-items > div:nth-child(".concat(i + 1, ")")).appendChild(removeBtn);
}
...
getData();
getFromLocalStorage(); // 'http://makeup-api.herokuapp.com/'
},{"./utils":"utils/index.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
